...
/* --- 运行时状态 --- */
let currentTone = null;       // 当前阵营音色: 'bright' | 'dark' | null
let currentFaction = null;    // 当前阵营: 'r' | 'b'
const lastFired = new Map();  // 节流记录
let activeHandles = [];       // 活跃的序列句柄
let nodeCount = 0;            // 活跃节点数估计
...
function buildImpulseResponse(stereo, duration) {
...
function makeBus(wet, pan, life, faction, busTarget) {
...
/** 节点释放 */
function scheduleRelease(nodes, lifeMs) {
  const ms = Math.max(60, lifeMs + 350);
  const collected = Array.isArray(nodes) ? nodes : Object.values(nodes).filter(n => n && typeof n.disconnect === 'function');
  setTimeout(() => {
...
function envReverse(param, t, peak, attack, decay) {
  const p = Math.max(peak, FLOOR * 2);
  param.setValueAtTime(FLOOR, t);
  param.exponentialRampToValueAtTime(p, t + attack);
  param.exponentialRampToValueAtTime(FLOOR, t + attack + decay);
...
function mkOsc(type, freq, t, dur, detune) {
 ...
 */
function leatherCreak(dest, t, cf, q, peak, dur, grain, pit) {
 ...
 */
function bladeClash(dest, t, base, peak, dur, grind, partialsKey, pit) {
 ...
 */
function armorClink(dest, t, n, f0, f1, peak, spread, ring, softAttack, forceHP, pit) {
 ...
 */
function woodKnock(dest, t, f, peak, dur, pit) {
 ...
 */
function horseSnort(dest, t, f, peak, dur, pit) {
 ...
 */
function dustScuff(dest, t, f0, f1, q, peak, dur, attack, pit) {
 ...
 */
function clothRustle(dest, t, cf, q, peak, dur, attack, pit) {
 ...
 * bannerFlap —— 旌旗拍打
 */
function bannerFlap(dest, t, n, cf, peak, pit, pan) {
  for (let i = 0; i < n; i++) {
 ...
 */
function crowdBed(dest, t, peak, lp, dur, pit) {
  const d = dur || 1.5;
 ...
 */
function stoneCrush(dest, t, pit) {
  // ① 木石相击
 ...
 */
function warDrum(dest, t, f0, f1, peak, dur, noSnap, attack, pit) {
  if (f0 <= 0) return;
 ...
 */
function transient(dest, t, freq, q, peak, dur, rate, pit) {
 ...
 */
function bronzeBody(dest, t, base, peak, decayScale, partialsKey, vibHz, pit) {
  const partials = PARTIALS[partialsKey];
  if (!partials) return;
 ...
 */
function chimeNote(dest, t, freq, peak, dur, pit) {
 ...
 */
function whoosh(dest, t, f0, f1, q, peak, dur, pit) {
  const n = mkNoise(t, dur + 0.05, rand(0.95, 1.05));
  const bp = mkFilter('bandpass', f0 * pit, q);
  bp.frequency.setValueAtTime(f0 * pit, t);
  bp.frequency.exponentialRampToValueAtTime(f1 * pit, t + dur);
 ...
 */
function footStep(dest, t, lo, tone, q, peak, dur, pit) {
...
function renderBeat(eventName, t, opts = {}) {
  ...
  const recipeWet = (recipe.opts && recipe.opts.wet) || 0;
  const life = (recipe.opts && recipe.opts.life) || 1.0;
...
/** 执行单条合成指令 */
function executeInst(inst, dest, t, pit, vol) {
  ...
  try {
    ...
    // 单次合成失败不中断整体
    console.warn('[SFX] executeInst failed:', inst.type, e.message);
  }
}
...
function scheduleSequence(sequence, baseT, opts = {}) {
  const id = ++sequenceId;
  let cancelled = false;
...
export const SFX = {
  ...
  captured(victimType, victimSide, opts) {
