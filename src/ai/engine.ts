...
import { searchBestMoveSliced, DIFFICULTY } from './search.ts';
import type { DifficultyPreset } from './search.ts';

export { DIFFICULTY };

export type AIEngineMode = 'worker' | 'sliced' | 'unknown';
...
export class AIEngine {
  ...
  _thinking: boolean;
  _cancelled: boolean;
  _probeTimer?: ReturnType<typeof setTimeout>;
  _slicedReason?: string;

  /**
   * @param {{difficulty?:number, onModeChange?:Function}} [opts]
   */
  constructor(opts: AIEngineOpts = {}) {
  ...
  setDifficulty(level: number): number {
  ...
  getDifficultyInfo(): DifficultyPreset { return DIFFICULTY[this.difficulty] || DIFFICULTY[2]!; }
   ...
   */
  async think(fen: string, side: string, opts: { depth?: number, timeLimit?: number, minDelay?: number, randomness?: number } = {}): Promise<unknown> {
  ...
  _thinkInWorker(fen: string, side: string, depth: number, timeLimit: number, randomness: number): Promise<unknown> {
...
export function createAIEngine(opts?: AIEngineOpts): AIEngine { return new AIEngine(opts); }
