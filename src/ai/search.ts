...
import { RED, BLACK, PT, opposite, PIECE_VALUE, PAWN_CROSSED_VALUE, FILES, RANKS } from '../core/constants.ts';
import { Board } from '../core/board.ts';
import {
  generateAllLegalMoves, generatePseudoMoves, makeMove, unmakeMove,
  isInCheck, hasCrossedRiver, type FullMove
} from '../core/rules.ts';
...
/** 把 10 行 × 9 列的二维表压平 */
const T = (rows: Array<Array<number>>): Int16Array => {
 ...
 * @param {string} side
 */
export function evaluate(board: Board, side: string): number {
  let scoreRed = 0;
...
/** MVV-LVA：优先吃大子、用小子吃 */
function scoreMove(board: Board, m: FullMove): number {
...
function orderedMoves(board: Board, side: string, ttMove: FullMove | null): FullMove[] {
  const moves = generateAllLegalMoves(board, side);
  for (let i = 0; i < moves.length; i++) {
    const m = moves[i]!;
    let s = scoreMove(board, m);
    if (ttMove && sameMove(m, ttMove)) s += 1000000;
    m._s = s;
  }
  moves.sort((a, b) => (b._s || 0) - (a._s || 0));
  return moves;
}

function sameMove(a: FullMove | null | undefined, b: FullMove | null | undefined): boolean {
...
function quiescence(board: Board, side: string, alpha: number, beta: number, ply: number, ctx: SearchCtx): number {
  ctx.nodes++;
  ...
  for (let i = 0; i < all.length; i++) {
    const m = all[i]!;
    if (m.capture) { m._s = scoreMove(board, m); caps.push(m); }
  }
  if (!caps.length) return stand;
  caps.sort((a, b) => (b._s || 0) - (a._s || 0));
...
function negamax(board: Board, side: string, depth: number, alpha: number, beta: number, ply: number, ctx: SearchCtx): number {
  ctx.nodes++;
  if ((ctx.nodes & 1023) === 0 && Date.now() > ctx.deadline) { ctx.timeout = true; return alpha; }

  if (depth <= 0) return quiescence(board, side, alpha, beta, ply, ctx);

  const moves = orderedMoves(board, side, null);
  if (!moves.length) {
    // 无子可动：将死或困毙，中国象棋均判负
 ...
 */
export function searchBestMove(board: Board, side: string, opts: SearchOpts = {}): SearchResult | null {
  const maxDepth = Math.max(1, opts.depth || 3);
  const timeLimit = opts.timeLimit || 1200;
  const randomness = opts.randomness || 0;
  const start = Date.now();
  const ctx: SearchCtx = { nodes: 0, deadline: start + timeLimit, timeout: false, maxQPly: 6 };

  let rootMoves = orderedMoves(board, side, null);
  ...
  for (let depth = 1; depth <= maxDepth; depth++) {
    let alpha = -Infinity;
    const beta = Infinity;
    let localBest: { m: FullMove, score: number } | null = null;
    const scored: Array<{ m: FullMove, score: number }> = [];
    ...
    if (localBest && (!ctx.timeout || !best)) {
      best = { from: localBest.m.from, to: localBest.m.to, score: localBest.score, depth, nodes: 0, elapsed: 0, timedOut: false };
      // 下一轮把本轮最优排到最前，提升剪枝效率
  ...
  if (!best) {
    const m = rootMoves[0]!;
    best = { from: m.from, to: m.to, score: 0, depth: 0, nodes: 0, elapsed: 0, timedOut: false };
  }
 ...
 */
export async function searchBestMoveSliced(board: Board, side: string, opts: SearchOpts = {}): Promise<SearchResult | null> {
  const maxDepth = Math.max(1, opts.depth || 3);
  const timeLimit = opts.timeLimit || 900;
  const randomness = opts.randomness || 0;
  const yieldFn = opts.yieldFn || (() => new Promise<void>(r => setTimeout(r, 0)));
  const start = Date.now();
  const ctx: SearchCtx = { nodes: 0, deadline: start + timeLimit, timeout: false, maxQPly: 4 };
...
export const DIFFICULTY: Record<number, DifficultyPreset> = {
