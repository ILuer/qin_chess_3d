...
import { createInputSystem } from './ui/input.ts';
import { createHUD } from './ui/hud.ts';
import { createControls } from './ui/controls.ts';
import { createAIEngine } from './ai/engine.ts';
import { createPieceMesh, disposePieceFactory, setPieceLod } from './render/pieceFactory.ts';
...
let sceneSys: any = null;      // SceneSystem
let effects: any = null;       // Effects
let input: any = null;         // InputSystem
let hud: any = null;           // HUD
let controls: any = null;      // Controls
let aiEngine: any = null;      // AIEngine
let gs: any = null;            // GameState
let followCam: any = null;     // FollowCamera（棋子检查跟随；T 键切换）
...
let reviewTimer: ReturnType<typeof setTimeout> | null = null;       // 自动播放定时器句柄
let reviewHidGameOver = false;// 进入复盘时是否隐藏了结束面板（退出时恢复）
let lastGameOverInfo: any = null;  // 结束面板内容缓存（复盘退出后重新展示，避免重复音效/埋点）
...
function rebuildPieces() {
  ...
  src.board.forEach((p: any, f: number, r: number) => {
 ...
 */
function applyMove(from: any, to: any): void {
...
function afterMove(rec: any, from: any, to: any): void {
  effects.showLastMoveMarker(from, to);
...
function pumpAI(dt: number): void {
 ...
 * @returns {number|null}
 */
function followFitRadiusFor(sel: any): number | null {
...
function previewMove(idx: number): void {
  if (!gs) return;
 ...
 */
function renderReview(cursor: number, len: number, sub: string, active: boolean): void {
  reviewActive = active;
  if (active) {
...
function enterReview(entryIndex?: number): void {
...
function exitReviewIfActive(): void {
  if (reviewActive) exitReview();
}

/** 走子记录双击：PLAY=进入复盘到该步之后；REVIEW=游标跳到该步之后 */
function onMoveLogDblClick(idx: number): void {
  if (reviewActive) { reviewCtrl.seek(idx + 1); return; }
  enterReview(idx + 1);
}

/** 导出棋谱入口（move-log「导出棋谱」按钮） */
function openExportPanelFlow(): void {
 ...
 */
function restoreFromSave(save: any): { restored: boolean, full: boolean } {
  let full = true;
...
const inputGame = {
  isLocked: (): boolean => reviewActive || animator.isBusy || aiBusy || gameOver,
  canControl: (side: string): boolean => !reviewActive && !gameOver && side === gs.sideToMove && (!aiEnabled || side === humanSide),
  pieceAt: (f: number, r: number): any => gs.pieceAt(f, r),
  legalMoves: (f: number, r: number): any => gs.getLegalMoves(f, r),
  blockedPoints: (f: number, r: number): any => gs.getBlockedPoints(f, r),
  whyIllegal: (from: any, to: any): any => gs.whyIllegal(from, to),
  meshAt: (f: number, r: number): any => (pieceMeshes[f] ? pieceMeshes[f]![r] : null),
  ...
  onMove: (from: any, to: any): void => applyMove(from, to),
  onIllegal: (from: any, to: any, reason: string, mesh: any): void => {
    if (mesh) animator.shakeMesh(mesh, 0.12, 0.3); // 明确拒绝反馈（Oil）
    SFX.play('illegal');
    hud.showToast(reason || '这步棋走不了', 'warn');
  },
  onHover: (mesh: any): void => {
...
function onFatalRejection(e: any): void {
...
async function boot(): Promise<void> {
  ...
  hud = createHUD({
    ...
    onReviewInterval: (ms: number): void => { if (reviewCtrl) reviewCtrl.setInterval(ms); }
  });
  ...
  try {
    ...
    sceneSys = await createSceneSystem(container!, {
      onQualityDrop: (fps: number): void => hud.showToast(`帧率偏低（约 ${fps}fps），已自动降低画质以保障流畅`, 'warn', 3.2)
    });
