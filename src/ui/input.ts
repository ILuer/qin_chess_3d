...
import { fromWorld, toWorld } from '../core/constants.ts';
...
export class InputSystem {
  ...
  selected: any;
  enabled: boolean;
  _down: { x: number, y: number, id: number, time: number } | null;
  _dragging: boolean;
  _hoverDirty: boolean;
  _hoverMesh: any;
  _hoverCell: any;
  _pointerInside: boolean;
  _onPointerDown: (ev: PointerEvent) => void;
  _onPointerMove: (ev: PointerEvent) => void;
  _onPointerUp: (ev: PointerEvent) => void;
   ...
   */
  constructor({ domElement, camera, piecesGroup, game }: { domElement: HTMLElement, camera: any, piecesGroup: any, game: any }) {
    this.dom = domElement;
    this.camera = camera;
  ...
  /** 客户端坐标 -> NDC */
  _toNdc(ev: PointerEvent, out: any): any {
  ...
  handleContextMenu(ev: Event): void {
    ev.preventDefault();
  ...
  handleClick(ndc: any): void {
    ...
    if (sel) {
      ...
      // 3) 合法落点 -> 走子
      const target = sel.moves.find((m: any) => m.file === cell.file && m.rank === cell.rank);
      if (target) {
  ...
  select(file: number, rank: number): boolean {
    const game = this.game;
  ...
  /** 当前选中的格子 */
  getSelection(): any { return this.selected; }
  ...
  _setHover(mesh: any, cell: any): void {
  ...
  update(): void {
    ...
    if (hit) {
      ...
      if (this.selected && this.selected.moves.some((m: any) => m.file === hit.cell.file && m.rank === hit.cell.rank)) {
    ...
    if (cell && this.selected && this.selected.moves.some((m: any) => m.file === cell.file && m.rank === cell.rank)) {
...
export function cellToWorld(file: number, rank: number): { x: number, z: number } { return toWorld(file, rank); }

export function createInputSystem(cfg: any): InputSystem { return new InputSystem(cfg); }
