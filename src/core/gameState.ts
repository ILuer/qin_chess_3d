...
/** 按阵营取纵线名 */
function fileName(file: number, side: string): string {
  return side === RED ? FILE_NAMES_RED[file]! : FILE_NAMES_BLACK[file]!;
}

/** 按阵营取数字（红=汉字，黑=阿拉伯） */
function numName(n: number, side: string): string {
 ...
 * @returns {string}
 */
export function getMoveNotation(board: Board, from: { file: number, rank: number }, to: { file: number, rank: number }): string {
...
export interface MoveRecord {
  ...
  captured: { type: string, side: string } | null;
  notation: string;
...
export class GameState {
  ...
  captured: Record<string, Array<{ type: string, side: string }>>;
   ...
   */
  constructor(board?: Board, sideToMove?: string) {
  ...
  static fromList(list: Parameters<typeof boardFromList>[0], sideToMove: string = RED): GameState {
    return new GameState(boardFromList(list), sideToMove);
  }

  static fromFen(fen: string, sideToMove?: string): GameState {
  ...
  /** 某格棋子 */
  pieceAt(file: number, rank: number): Piece | null { return this.board.get(file, rank); }

  /** 合法走法（已过滤自杀与白脸将） */
  getLegalMoves(file: number, rank: number): Move[] { return generateLegalMoves(this.board, file, rank); }

  /** 被蹩马腿 / 塞象眼 / 过河限制阻挡的点 */
  getBlockedPoints(file: number, rank: number) { return getBlockers(this.board, file, rank); }

  /** 走法非法原因（null = 合法） */
  whyIllegal(from: { file: number, rank: number }, to: { file: number, rank: number }): string | null {
    return explainIllegal(this.board, from.file, from.rank, to.file, to.rank);
  }

  /** 当前方是否被将军 */
  inCheck(side: string = this.sideToMove): boolean { return isInCheck(this.board, side); }
  ...
  lastMove(): MoveRecord | null { return this.history.length ? this.history[this.history.length - 1]! : null; }

  /** 当前回合数（从 1 开始） */
  get moveNumber(): number { return Math.floor(this.history.length / 2) + 1; }
   ...
   */
  move(from: { file: number, rank: number }, to: { file: number, rank: number }, opts: { force?: boolean } = {}): { ok: boolean, error?: string, record?: MoveRecord } {
    ...
    if (captured) {
      this.halfMoveClock = 0;
      this._drawHinted = false;
      (this.captured[captured.side] || (this.captured[captured.side] = [])).push({ type: captured.type, side: captured.side });
    } else {
      this.halfMoveClock++;
    }
   ...
   */
  undo(steps: number = 1): MoveRecord[] {
    ...
    for (let i = 0; i < steps; i++) {
      ...
      if (rec.captured) {
        const bucket = this.captured[rec.captured.side] || (this.captured[rec.captured.side] = []);
  ...
  /** 重开一局 */
  reset(board?: Board, sideToMove: string = RED): void {
    this.board = board || createInitialBoard();
  ...
  /** 议和 / 判和 */
  drawGame(reason: string = END_REASON.DRAW_AGREE): boolean {
    if (this.isGameOver()) return false;
    this.status = 'draw';
    this.winner = null;
  ...
  /** 走子记录（UI 列表用），按回合分组 */
  getMoveLog(): Array<{ no: number, red: string, black: string, redIndex: number, blackIndex: number }> {
    const rows: Array<{ no: number, red: string, black: string, redIndex: number, blackIndex: number }> = [];
  ...
  /** 所有合法走法（AI / 状态判定用） */
  allLegalMoves(side: string = this.sideToMove): FullMove[] { return generateAllLegalMoves(this.board, side); }
   ...
   * @returns {string} 如 'h2'
   */
  static toUcci(file: number, rank: number): string {
  ...
  /** 轻量快照（传给 Worker） */
  snapshot(): { fen: string, sideToMove: string, halfMoveClock: number } {
