...
/** 复盘子状态 */
export type ReviewSub = 'idle' | 'playing';
...
export class ReviewController {
  _getHistory: () => Array<unknown>;
  _onChange: (cursor: number, len: number, sub: ReviewSub, active: boolean) => void;
  interval: number;
  _timer: ReviewTimer | null;
  active: boolean;
  cursor: number;
  sub: ReviewSub;
  ...
  constructor({ getHistory, onChange, interval = 1000, timer = null }: {
    ...
    onChange?: (cursor: number, len: number, sub: ReviewSub, active: boolean) => void;
    interval?: number;
    timer?: ReviewTimer | null;
  }) {
  ...
  /** 历史长度（半回合数） */
  get len(): number { return this._getHistory().length; }
   ...
   */
  enter(entryIndex?: number): boolean {
  ...
  _pauseAndSeek(target: number): void {
    this._stopTimer();
    this.sub = 'idle';
