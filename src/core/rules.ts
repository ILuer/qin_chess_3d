...
import type { Board, Piece } from './board.ts';
...
/** 是否在某方九宫内 */
export function inPalace(file: number, rank: number, side: string): boolean {
...
export function isOwnHalf(rank: number, side: string): boolean {
  return side === RED ? rank >= 5 : rank <= 4;
}

/** 兵/卒是否已过河 */
export function hasCrossedRiver(rank: number, side: string): boolean {
  return side === RED ? rank <= 4 : rank >= 5;
}

/** 该方向前的 rank 增量：红方向 rank 递减方向前进 */
export function forwardDir(side: string): number {
  return side === RED ? -1 : 1;
}

/** state/board 兼容形态：rules 入参既可以是 { board, ... } 也可以直接是 Board */
type StateLike = Board | { board: Board };

/**
 * 兼容 state 与裸 Board：rules 的入参既可以是 { board, ... } 也可以直接是 Board
 * @returns {import('./board.ts').Board}
 */
function getBoard(state: StateLike): Board {
  if (!state) throw new Error('rules: state/board 不能为空');
  return (state as { board?: Board }).board ? (state as { board: Board }).board : state as Board;
}
...
const HORSE_DELTAS: Array<[number, number]> = [
  [1, 2], [2, 1], [2, -1], [1, -2],
  [-1, -2], [-2, -1], [-2, 1], [-1, 2]
];
 ...
 */
export function makeMove(board: Board, ff: number, fr: number, tf: number, tr: number): Piece | null {
...
/** 撤销 makeMove */
export function unmakeMove(board: Board, ff: number, fr: number, tf: number, tr: number, captured: Piece | null): void {
...
type MoveList = Array<Move>;

/** 把一个目标点加入结果（若为己方子则跳过） */
function pushIfOk(board: Board, out: MoveList, side: string, f: number, r: number): boolean {
...
/** 将/帅：九宫内单步直行 */
function genKing(board: Board, f: number, r: number, side: string, out: MoveList): void {
...
/** 士/仕：九宫内单步斜行 */
function genAdvisor(board: Board, f: number, r: number, side: string, out: MoveList): void {
...
/** 象/相：田字，塞象眼不可走，不可过河 */
function genElephant(board: Board, f: number, r: number, side: string, out: MoveList): void {
  for (const [df, dr] of DIAG) {
...
/** 马：日字，蹩马腿 */
function genHorse(board: Board, f: number, r: number, side: string, out: MoveList): void {
...
/** 车：直线任意步，路径不可有子 */
function genRook(board: Board, f: number, r: number, side: string, out: MoveList): void {
  for (const [df, dr] of ORTHO) {
    let nf = f + df, nr = r + dr;
    while (inBoard(nf, nr)) {
...
function genCannon(board: Board, f: number, r: number, side: string, out: MoveList): void {
  for (const [df, dr] of ORTHO) {
    let nf = f + df, nr = r + dr;
    // 阶段一：空格随便走
...
function genPawn(board: Board, f: number, r: number, side: string, out: MoveList): void {
...
/**
 * 生成伪合法走法（未过滤自杀与白脸将）
 * @param {import('./board.ts').Board} board
 * @returns {Array<{file:number, rank:number, capture:boolean}>}
 */
export function generatePseudoMoves(board: Board, file: number, rank: number): MoveList {
...
function isPawnAt(board: Board, f: number, r: number, side: string): boolean {
 ...
 * 士与象永远够不到对方九宫，故不参与判定。
 */
export function isAttacked(board: Board, f: number, r: number, bySide: string): boolean {
  // —— 1. 四正方向扫描：车 / 炮 / 将（白脸将） ——
  for (let d = 0; d < 4; d++) {
    const dir = ORTHO[d]!;
    const df = dir[0], dr = dir[1];
    let x = f + df, y = r + dr;
    let first: Piece | null = null, fx = 0, fy = 0;
    while (inBoard(x, y)) {
 ...
 */
export function isFacingKings(state: StateLike): boolean {
 ...
 * @param {string} side
 */
export function isInCheck(state: StateLike, side: string): boolean {
  const board = getBoard(state);
 ...
 * @returns {Array<{file:number, rank:number, capture:boolean}>}
 */
export function generateLegalMoves(state: StateLike, file: number, rank: number): MoveList {
  const board = getBoard(state);
  const piece = board.get(file, rank);
  if (!piece) return [];
  const pseudo = generatePseudoMoves(board, file, rank);
  const legal: MoveList = [];
 ...
 */
export function generateAllLegalMoves(state: StateLike, side: string): Array<FullMove> {
  const board = getBoard(state);
  const out: Array<FullMove> = [];
...
export function hasAnyLegalMove(state: StateLike, side: string): boolean {
  const board = getBoard(state);
...
/** 判断一步走法是否合法 */
export function isLegalMove(state: StateLike, ff: number, fr: number, tf: number, tr: number): boolean {
  const moves = generateLegalMoves(state, ff, fr);
  for (let i = 0; i < moves.length; i++) {
    const m = moves[i]!;
    if (m.file === tf && m.rank === tr) return true;
  }
  return false;
}
 ...
 */
export function explainIllegal(state: StateLike, ff: number, fr: number, tf: number, tr: number): string | null {
  const board = getBoard(state);
 ...
 */
export function getBlockers(state: StateLike, file: number, rank: number): Array<{ file: number, rank: number, reason: 'leg' | 'eye' | 'river' }> {
  const board = getBoard(state);
  const p = board.get(file, rank);
  if (!p) return [];
  const out: Array<{ file: number, rank: number, reason: 'leg' | 'eye' | 'river' }> = [];
 ...
 */
export function getGameStatus(state: StateLike, sideToMove: string): 'playing' | 'check' | 'checkmate' | 'stalemate' {
  const board = getBoard(state);
...
export function getCheckedKing(state: StateLike, side: string): { file: number, rank: number } | null {
  const board = getBoard(state);
 ...
 */
export function getCheckingPieces(state: StateLike, side: string): Array<{ file: number, rank: number, type: string }> {
  const board = getBoard(state);
  const k = board.findKing(side);
  if (!k) return [];
  const foe = opposite(side);
  const out: Array<{ file: number, rank: number, type: string }> = [];
  board.forEachSide(foe, (p, f, r) => {
    if (p.type === PT.ADVISOR || p.type === PT.ELEPHANT) return;
    const moves = generatePseudoMoves(board, f, r);
    for (let i = 0; i < moves.length; i++) {
      const m = moves[i]!;
      if (m.file === k.file && m.rank === k.rank) { out.push({ file: f, rank: r, type: p.type }); break; }
    }
  });
  return out;
}
