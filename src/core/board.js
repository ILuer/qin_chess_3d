...
export class Board {
  constructor(cells) {
    /** @type {Array<{type:string, side:string}|null>} */
    this.cells = cells || new Array(CELLS).fill(null);
  }

  /** 棋盘坐标 -> 索引 */
  static index(file, rank) { return rank * FILES + file; }

  /** 是否在棋盘内 */
  static inBoard(file, rank) {
  ...
  get(file, rank) {
    ...
    return this.cells[rank * FILES + file];
  }
  ...
  /** 该格是否为空 */
  isEmpty(file, rank) { return this.get(file, rank) === null; }
   ...
   */
  sameFileSameType(file, type, side) {
    const out = [];
 ...
 */
export function boardFromList(list) {
  const board = new Board();
