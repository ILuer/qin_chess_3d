...
const _textures = new Set<any>();
const _extraMats = new Map<string, any>();

function track(tex: any): any { _textures.add(tex); return tex; }
...
function toTexture(canvas: any, opts: Record<string, any> = {}): any {
  const {
    repeat = [1, 1],
    srgb = true,
    aniso = 1,
...
/** 单个方形回旋（雷纹基元） */
function drawSpiral(ctx: any, cx: number, cy: number, s: number, mirror: boolean): void {
  ctx.save();
  ctx.translate(cx, cy);
...
/** 一条云雷纹饰带 */
function drawLeiWenBand(ctx: any, x: number, y: number, w: number, h: number, color: string, lw: number, unit: number): void {
  ctx.save();
 ...
 */
export function createLeiWenTexture(opts: Record<string, any> = {}): any {
  const {
    w = 512,
    h = 128,
    bg = PALETTE.xuan,
 ...
 * 程序化木纹（棋盘台面、战车/抛石机木构）
 */
export function createWoodTexture(opts: Record<string, any> = {}): any {
  const {
    w = 1024,
    h = 1024,
    base = PALETTE.wood,
 ...
 * 旗面贴图：云雷纹边框 + 中央大字
 */
export function createBannerTexture(glyph: string, side: string, opts: Record<string, any> = {}): any {
...
function std(color: any, roughness: number, metalness: number, extra?: Record<string, any>): any {
...
function buildLibrary(): any {
  ...
  return {
    ...
    /** 便捷：按阵营取材质集 */
    side(id: string): any { return id === 'r' ? this.r : this.b; }
  };
}
