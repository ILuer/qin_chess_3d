...
import { PALETTE, toWorld } from '../core/constants.js';
...
export class Effects {
  /**
   * @param {import('./scene.js').SceneSystem} sceneSys
   */
  constructor(sceneSys) {
  ...
  _reg(m) { this._materials.push(m); return m; }
  ...
  clearHints() { disposeChildren(this.hintGroup, false); }
  ...
  clearBlockedHints() { disposeChildren(this.blockedGroup, false); }
   ...
   */
  showLastMoveMarker(from, to) {
    this.clearLastMoveMarker();
    if (!from || !to) return;
    const mk = (pos, mat, inner, outer) => {
   ...
   * @param {Object} [opts]
   */
  spawnImpactParticles(position, color = PALETTE.liuJin, opts = {}) {
  ...
  /** 扩散涟漪圆环 */
  spawnRipple(position, color = PALETTE.liuJinLight, life = 0.6) {
   ...
   */
  checkPulse(strength = 0.55, duration = 0.85) {
 ...
 */
function disposeChildren(group, disposeGeometry) {
...
export function createEffects(sceneSys) { return new Effects(sceneSys); }
