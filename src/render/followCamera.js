...
import { toWorld } from '../core/constants.js';
...
export class FollowCamera {
  ...
  get mode() { return (this.enabled && this._following) ? 'follow' : 'fixed'; }

  /** 是否正处于跟随目标中 */
  get isFollowing() { return this._following; }
  /** 是否正在自动收敛半径 */
  get isRadiusAuto() { return this._radiusAuto; }
  get radiusTarget() { return this._radiusTarget; }
  get exitPending() { return this._exitPending; }
  ...
  _maintainLimits(controls, actualRadius, radius) {
    const cfg = this.config;
...
export function createFollowCamera(cfg) { return new FollowCamera(cfg); }
 ...
 */
export function computeFixedViewFitRadius({ fov, aspect, dir, target, baseRadius }) {
 ...
 */
export function computeFitRadius({ fov, aspect, dir, target, points, minR = 0.8, maxR = 26, margin = 0.05 }) {
 ...
 * @returns {number} 适配半径
 */
export function computeFollowFitRadius({ camera, controls, moves, piece, margin }) {
