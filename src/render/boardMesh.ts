...
const wx = (file: number) => (file - 4) * GRID;
const wz = (rank: number) => (rank - 4.5) * GRID;
...
class Bin {
  ...
  push(g: any): this { this.geoms.push(g); return this; }
  /** 轴对齐线段盒 */
  seg(x1: number, z1: number, x2: number, z2: number, w: number): this {
...
/** 传统"十"字直角位标（炮位 / 兵位） */
function addPositionMark(bin: Bin, file: number, rank: number): void {
...
export function createBoard(): any {
  ...
  /* ---- dispose ---- */
  group.userData.dispose = function (): void {
...
function buildBrazier(mats: any, bin: Bin, flames: any[], x: number, z: number): any {
  const B = mats.board;
  const K = mats.common;
  // 础
...
function buildBanner(mats: any, side: string, x: number, z: number, bins: any): any {
  ...
  const pushGeom = (bin: any, geo: any) => {
...
export function createEnvironment(): any {
  const mats = getMaterials();
  const B = mats.board;
  const group = new THREE.Group();
  ...
  const mkBannerGroup = (side: string, x: number, z: number) => buildBanner(mats, side, x, z, bannerBins);
  mkBannerGroup('r', -6.10, 3.60);
  mkBannerGroup('r', 6.10, 3.60);
  mkBannerGroup('b', -6.10, -3.60);
  mkBannerGroup('b', 6.10, -3.60);

  const mergedBannerMeshes: any[] = [];
  const mkMerged = (bin: Bin, material: any, receiveShadow: boolean) => {
    const geo = bin.merge();
    if (!geo) return;
  ...
  group.userData.update = function (t: number): void {
