...
export const PIECE_GLYPH: Record<string, Record<string, string>> = {
...
const _lod = (table: Record<string, number>, v: number): number => (_lodLevel >= 1 && table[v] !== undefined) ? table[v]! : v;

const cyl = (rt: number, rb: number, h: number, seg = 12): any => new THREE.CylinderGeometry(rt, rb, h, _lod(_lodSeg.cyl, seg), 1);
const box = (w: number, h: number, d: number): any => new THREE.BoxGeometry(w, h, d);
const sph = (r: number, w = 12, h = 10): any => new THREE.SphereGeometry(r, _lod(_lodSeg.sphW, w), _lod(_lodSeg.sphH, h));
const dome = (r: number, w = 12, h = 7, frac = 0.6): any =>
  new THREE.SphereGeometry(r, _lod(_lodSeg.domeW, w), _lod(_lodSeg.domeH, h), 0, Math.PI * 2, 0, Math.PI * frac);
const tor = (R: number, t: number, rs = 6, ts = 18): any => new THREE.TorusGeometry(R, t, rs, _lod(_lodSeg.torTS, ts));

/* 关键剪影件「不降段」变体（lod-spec §3 红线）——任何 LOD 级别保持现状段数。
 * 对应 R 车轮（tor/cyl/sph）、K 鹖冠（cyl/sph/strut）、K 王座（cyl/sph/strut）、C A 架（strut）。 */
const cylKeep = (rt: number, rb: number, h: number, seg = 12): any => new THREE.CylinderGeometry(rt, rb, h, seg, 1);
const sphKeep = (r: number, w = 12, h = 10): any => new THREE.SphereGeometry(r, w, h);
const domeKeep = (r: number, w = 12, h = 7, frac = 0.6): any =>
  new THREE.SphereGeometry(r, w, h, 0, Math.PI * 2, 0, Math.PI * frac);
const torKeep = (R: number, t: number, rs = 6, ts = 18): any => new THREE.TorusGeometry(R, t, rs, ts);

/** 轻微弯曲的旗面（PlaneGeometry + 顶点位移，仍为 indexed 几何体） */
function curvedBanner(w: number, h: number, bend: number, segs = 8): any {
...
function _sampleMap(tex: any): any {
...
class Parts {
  ...
  add(geom: any, mat: any, opts: any): this {
    const o = opts || {};
  ...
  strut(mat: any, a: any, b: any, rTop: number, rBot: number, seg: number = 12): this {
    return this._strutImpl(mat, a, b, rTop, rBot, seg, true);
  }

  /** 关键剪影件「不降段」变体（lod-spec §3 红线）：R 车轮辐条/K 鹖冠立缨/C A 架斜梁等保持现状段数 */
  strutKeep(mat: any, a: any, b: any, rTop: number, rBot: number, seg: number = 12): this {
    return this._strutImpl(mat, a, b, rTop, rBot, seg, false);
  }

  _strutImpl(mat: any, a: any, b: any, rTop: number, rBot: number, seg: number, applyLod: boolean): this {
  ...
  build(): any {
    ...
    const out: any[] = [];
    const emit = (geos: any[], mat: any): void => {
...
class MultiParts {
  ...
  /** 默认收集器（不归属动画子组的公共零件） */
  get base(): any { return this.get(this.defaultName); }

  /** 全部构建 -> { groupName: [THREE.Mesh][] } */
  buildAll(): Record<string, any> {
...
function buildChariot(mp: any, M: any, K: any, side: string): void {
  ...
  Pc.add(
    // D6：车旗 plane 修正为贴图 0.75 等比（原 0.175×0.130=1.35 非等比→汉字压扁）。
    // 面积保持 0.02279 ≈ 原 0.02275（整体缩放补偿，远观体量不变）；旗面 z 范围
    // 收窄后与旗杆（z≈0.118 表面）间隙 0.0145，原轻微穿模一并消除。
    curvedBanner(0.131, 0.174, 0.026, 8),
...
/** 单匹战马（复用给车用，缩放版） */
function buildOneHorse(P: any, M: any, K: any, ox: number, oz: number, scl: number): void {
  const s = scl || 1.0;
  const hide = M.leather;
  // 马身
...
function buildCannon(mp: any, M: any, K: any, side: string): void {
  ...
  Pt.add(
    // D6：炮旗 plane 修正为贴图 0.75 等比（原 0.120×0.096=1.25 非等比→汉字压扁）。
    // 面积保持 0.01153 ≈ 原 0.01152；旗面 z 范围收窄后与旗杆（z≈0.176 表面）
    // 间隙 0.0035，原轻微穿模一并消除。旗面顶 0.374 略高于旗杆尖 0.353（+0.021），
    // 属弯曲飘旗自然形态，可接受。
    curvedBanner(0.093, 0.124, 0.020, 6),
...
function buildKing(mp: any, M: any, K: any, side: string): void {
...
function buildKingArm(P: any, M: any, K: any): void {
 ...
 */
function buildKingCrown(P: any, M: any, K: any, BODY_BOT: number): void {
...
function buildKingSword(P: any, M: any, K: any, BODY_BOT: number): void {
...
function buildKingBanner(P: any, M: any, K: any, side: string): void {
 ...
 */
export function setPieceLod(group: any, lodLevel: number): boolean {
 ...
 */
function _collectSubgroupMeshes(root: any): any {
  ...
  for (const g of subgroups) {
    const arr: any[] = [];
    g.traverse((o: any) => { if (o.isMesh) arr.push(o); });
    map.set(g.name, arr);
  }
  return map;
}
 ...
 */
export function disposePieceFactory(): void {
